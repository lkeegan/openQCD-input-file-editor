...
#
# Module containing the bulk of functions.
#
...
def splitField(field, form):
    # Takes a widget, returns section and variable name based on
    # the widget object name of the form:
    # section____variable
...
def getVarTxt(field, form):
    # Get and display contents of
    # a txt field from config
...
def getVarCmb(field, form):
    # Set contents of combobox to value
    # in config (match on first word)
    sec, var = splitField(field, form)
    # special cases for multiple widget fields in action:
    split_vars = [
        "imuX0", "imuX1", "ispX0", "ispX1",
        ...
        if field.isEnabled():
            if form.config.has_option(sec, var.split("X")[0]):
                for row in range(field.count()):
                    try:
                        strCmb = str(field.itemText(row)).split()
                        tmpCnfg = str(form.config.get(sec, var.split("X")[0]))
                        strCnfg = tmpCnfg.split()[int(var.split("X")[1])]
                        if strCmb[0] == strCnfg:
                            field.setCurrentIndex(row)
                            return True
                    except:
                        print("ERR getVarCmb",
                              splitField(field, form), strCmb, strCnfg
                              )
        return False
    # generic code for the rest of the fields
        ...
        if form.config.has_option(sec, var):
            for row in range(field.count()):
                strCmb = str(field.itemText(row)).split()
                strCnfg = form.config.get(sec, var)
                if strCmb[0] == strCnfg:
                    field.setCurrentIndex(row)
                    return True
        return False
...
def setVarCmb(field, form):
    # Set variable in config to
    # (first word of) combobox selection
    sec, var = splitField(field, form)
    # special cases for multiple widget fields in action:
        ...
        if form.ui.Action____imuX0.isEnabled():
            if form.ui.Action____imuX1.isEnabled():
                txt = str(form.ui.Action____imuX0.currentIndex()) + " " + \
                    str(form.ui.Action____imuX1.currentIndex())
            else:
                txt = str(form.ui.Action____imuX0.currentIndex())
            if not form.config.has_section(sec):
                form.config.add_section(sec)
        ...
        if form.ui.Action____ispX0.isEnabled():
            if form.ui.Action____ispX1.isEnabled():
                txt = str(form.ui.Action____ispX0.currentIndex()) + " " + \
                    str(form.ui.Action____ispX1.currentIndex())
            else:
                txt = str(form.ui.Action____ispX0.currentIndex())
            if not form.config.has_section(sec):
                form.config.add_section(sec)
...
def getIndex(listWidget):
    ...
    try:
        strText = str(listWidget.item(listWidget.currentIndex().row()).text())
        return int(strText.split()[0])
    except:
...
def populateMDInt(form):
    ...
    for row in range(0, form.ui.listActions.count()):
        numAction = int(str(form.ui.listActions.item(row).text()).split()[0])
        list_actions.append(numAction)
    list_MDactions = []
    ...
    firstMDactionList = form.ui.tabMDint.findChildren(QtGui.QListWidget)[0]
    for action in list_actions:
        if action not in list_MDactions:
            firstMDactionList.addItem(str(action))


def generateInputFile(form):
    # Generate string containing input file
    # based on data in config

    # add HMC parameters to config that
    # are implied by the other parameters.
    strA = []
    listActionLevels = form.ui.tabMDint.findChildren(QtGui.QListWidget)
    nlv = len(listActionLevels)
    for lst in listActionLevels:
        setVarList(lst, form)
    ...
    if form.ui.radioSortDefault.isChecked():
        # default ordering
        defaultOrder = ["Run", "Directories", "Lattice",  "Boundary",
