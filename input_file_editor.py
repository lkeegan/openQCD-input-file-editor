...
class MainGUI(QtGui.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.listDbleFields = [self.ui.Lattice_parameters____beta,
        ...
        self.listIntFields = [self.ui.Random_number_generator____seed,
        ...
        self.listListDoubleFields = [self.ui.Lattice_parameters____kappa,
        ...
        for field in self.listIntFields:
    ...
    def initSignalsAndSlots(self):
        ...
        c = self.ui.Boundary_conditions____type
        c.currentIndexChanged.connect(self.changecmbBCS)
        c = self.ui.Solver____solver
        c.currentIndexChanged.connect(self.changecmbSolver)
        c = self.ui.Action____action
        c.currentIndexChanged.connect(self.changecmbAction)
    ...
    def changecmbSolver(self):
        ...
        if self.ui.listSolvers.count() > 0:
            ...
            tmpW = self.ui.listSolvers.item(self.ui.listSolvers.currentRow())
            tmpS = str(self.ui.Solver____solver.currentText())
            tmpText = str(index) + " [" + tmpS + "]"
            tmpW.setText(tmpText)
    ...
    def changecmbAction(self):
        ...
        if self.ui.listActions.count() > 0:
            ...
            txtForce = str(self.ui.Action____action.currentText())
            # update force parameters
            self.config.set("Force " + index, "force", "FRF" + txtForce[3:])
            # update action description in listbox
            tmpW = self.ui.listActions.item(self.ui.listActions.currentRow())
            tmpW.setText(index + " [" + txtForce + "]")
    ...
    def changecmbMDint(self):
        ...
        level = int(g.getItemPosition(g.indexOf(self.sender()))[1])
        if cmb.currentIndex() == 1:
            # show lambda field
            g.itemAtPosition(2, level).widget().setEnabled(True)
        else:
            # hide lambda field
            g.itemAtPosition(2, level).widget().setEnabled(False)
        utils.setVarTxt(g.itemAtPosition(2, level).widget(), self)
    ...
    def addSolver(self, index=99, sectionExists=False):
        # If sectionExists is True, then adds an existing
    ...
    def addRAT(self, index=99, sectionExists=False):
        # If sectionExists is True, then adds an existing (in the config)
        # solver with the supplied index.
        # If not then makes a new solver with a new index
    ...
    def addIntLevel(self, index=99, sectionExists=False):
        ...
        self.listDbleFields.append(txt)
        txt.editingFinished.connect(self.changeText)
        self.ui.gridLayout.addWidget(txt, 2, self.MDIntLevels, 1, 1)
        if utils.getVarTxt(txt, self) is False:
            txt.setText("0.19")
            utils.setVarTxt(txt, self)
        txt = QtGui.QLineEdit(self.ui.tabMDint)
        ...
        if utils.getVarCmb(cmb, self) is False:
        ...
        strLevelConfig = "Level " + str(self.MDIntLevels - 1)
        if self.config.has_option(strLevelConfig, "forces"):
            s = str(self.config.get(strLevelConfig, "forces"))
            for i in s.split():
                list.addItem(i)
    ...
    def removeIntLevel(self):
        ...
        if self.MDIntLevels > 1:
            g = self.ui.gridLayout
            wOld = g.itemAtPosition(4, self.MDIntLevels).widget()
            for i in range(wOld.count()):
                wNew = g.itemAtPosition(4, self.MDIntLevels - 1).widget()
                wNew.addItem(wOld.item(i).text())
        # Update config
        utils.setVarALL(self)
        # remove integration level from gui
        for x in range(0, 5):
            w = self.ui.gridLayout.itemAtPosition(x, self.MDIntLevels).widget()
            self.ui.gridLayout.removeWidget(w)
            w.setParent(None)
        self.MDIntLevels -= 1
        if self.MDIntLevels == 1:
    ...
    def changeText(self):
        # called when user changes a text field.
        # performs params consistency check &
        # updates edited text field in config
        consistent = 0
        utils.setVarTxt(self.sender(), self)
        tE = "Must be a list of positive doubles (e.g. 2.123 5e-3 0.1243)"
        for txt in self.listListDoubleFields:
            consistent += consistency.isListOfPositiveDoubles(txt, tE)
        tE = "Must be a positive double (e.g. 0.165, or 1.3e-1)"
        for txt in self.listDbleFields:
            consistent += consistency.isPositiveDouble(txt, tE)
        tE = " must be an integer multiple of "
        consistent += consistency.isIntegerMultiple(
            self.ui.Wilson_flow____nstep,
            self.ui.Wilson_flow____dnms,
            "Numer of integration steps" + tE + "measurement frequency")
        consistent += consistency.isIntegerMultiple(
            self.ui.MD_trajectories____dtr_ms,
            self.ui.MD_trajectories____dtr_log,
            "Measurement frequency" + tE + "log freqency")
        consistent += consistency.isIntegerMultiple(
            self.ui.MD_trajectories____dtr_cnfg,
            self.ui.MD_trajectories____dtr_ms,
            "Configuration save frequency" + tE + "measurement freqency")
        tE = " must be an integer multiple of configuration save freqency"
        consistent += consistency.isIntegerMultiple(
            self.ui.MD_trajectories____nth,
            self.ui.MD_trajectories____dtr_cnfg,
            "Number of MD Thermalisation trajectories" + tE)
        consistent += consistency.isIntegerMultiple(
            self.ui.MD_trajectories____ntr,
            self.ui.MD_trajectories____dtr_cnfg,
            "Number of MD trajectories" + tE)
        consistent += consistency.isListOfNPositiveInts(
            self.ui.SAP____bs,
            4,
